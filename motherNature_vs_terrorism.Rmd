...
```{r}
ggsave("plot.png", plot1,width=9, height=12.6)
```
...
deaths_disasters %>%
  ...
  arrange(Year) %>%
  # Use fct to order how disaster types are displayed on the char
...
```{r}
plot2 <- global_death_disasters %>%
  ggplot(aes(x = Year, y = Type, size=Deaths, color=Type, text = text)) +
  geom_point(alpha = 0.5) +
  scale_size(range = c(0.9, 13), name = "Death toll") +
  scale_color_manual(values = c("Earthquake" = "#d2601a",
                                ...
                                "Storm" = "#1d3c45",
                                "Volcanic" = "darkgreen")) +
  scale_x_continuous(n.breaks = 10, expand = c(0, 0), limits = c(1897, 2021)) +
  ...
  theme(axis.text.x = element_text(color="#000000"),
        axis.text.y = element_text(vjust=0.5, hjust=1, color="#000000"),
        legend.position = "none",
        text = element_text(color = "gray12", family = "serif"),
        plot.title = element_text(face="bold", size = 16, hjust = -1,
                                  color="#000000"),
        plot.subtitle = element_text(size = 10, face = "italic", hjust = -0.7),
