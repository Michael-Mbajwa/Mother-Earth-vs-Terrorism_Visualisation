...
Load the datasets
```{r}
fatalities <- read_csv("/Users/mac/My-R-graph-gallery/motherNature_vs_terrorism/data/fatalities-from-terrorism.csv")

disasters <- read_csv("/Users/mac/My-R-graph-gallery/motherNature_vs_terrorism/data/number-of-deaths-from-natural-disasters.csv")
```
...
# We are now going to create a circular lollipop plot that shows the yearly number of deaths around the world and per continent. The two plots at the end will be combined together to form a dashboard.

To start, lets get the fatalities by continent, group by year so as to ensure it tallies with the data from the world section.
```{r}
fatalities %>%
  filter((Entity %in% c("Australasia & Oceania", "Central America & Caribbean", "Central Asia", "East Asia", "Eastern Europe", "Middle East & North Africa", "North America", "South America", "South Asia", "Southeast Asia", "Sub-Saharan Africa", "Western Europe", "Western Sahara"))) %>%
  group_by(Year) %>%
  summarise(Deaths = sum(deaths)) -> total_deaths_continent

total_deaths_continent
```

```{r}
diff_deaths2 <- tibble(year = world_fatality$Year,
                      deaths_world = world_fatality$deaths,
                      deaths_continent = total_deaths_continent$Deaths,
                      diff = deaths_world - deaths_continent)

diff_deaths2
```
Conclusion: Both groups of data tally.
...
fatalities %>%
  ...
  mutate(Continent = case_when(
    str_detect(Entity, "Asia") ~ "Asia",
    str_detect(Entity, "Europe") ~ "Europe",
    TRUE ~ Entity
  )) %>%
  group_by(Continent, Year) %>%
  summarise(Deaths = sum(deaths)) -> fatality_by_continents
```
